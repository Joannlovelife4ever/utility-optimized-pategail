...
class replay_buffer(object):
    ...
    def sample(self, batch_size):
        if len(self.memory) > 0 and len(self.memory[0]) == 10:
            self.process()
        ...
        history_pos_concat = np.concatenate(history_pos, axis=0)
        
        return (np.concatenate(pos, 0), 
                np.concatenate(times, 0), 
                history_pos_concat,  # Fixed: concatenate on axis 0
                np.concatenate(home_point, 0), 
                np.concatenate(pre_pos_count, 0), 
                np.concatenate(stay_time, 0), 
                actions, 
                returns, 
                advantages)
