...
class gail(object):
    def __init__(self, env, file, seed, beta, noise, config_path='./config/config.yaml', eval=False):
        ...
        # Add sensitivity module parameters
        self.use_adaptive_privacy = True  
        self.min_sensitivity = 0.01  
        self.sensitivity_scaling = 1.0  
        ...
        if self.eval:
            self.policy_net.load_state_dict(torch.load(f'./results/result_{self.seed}_{self.noise}_{self.beta}/models_{1}/policy_net.pkl')) #############################
    ...
    def calculate_sensitivity(self, discriminator_output):
        sensitivity = discriminator_output
        min_sensitivity = 0.01
        sensitivity = torch.clamp(sensitivity, min=min_sensitivity)
        return sensitivity
    
    def reset_episode_budgets(self):
        """Call this at the start of each new episode"""
        if hasattr(self, 'episode_initialized'):
            delattr(self, 'episode_initialized')
        if hasattr(self, 'current_episode_budgets'):
            delattr(self, 'current_episode_budgets')
        if hasattr(self, 'episode_budget_used'):
            delattr(self, 'episode_budget_used')
    ...
    def one_reward(self, data_list, reward_input_dict, result_queue):
        """
        Corrected version: Distribute full episode budget across all samples
        Budget per episode: 4253.568 divided among 83 discriminators based on sensitivity
        """
        all_result = {}
        episode_budget = 4253.568 #noise 0.01, total budget: 638035.1336
        #episode_budget = 398.2416 #noise 0.1, total budget: 59736.2378
        #episode_budget = 41.80 #noise 1, total budget: 6270.6525
        
        if not hasattr(self, 'current_episode_budgets') or not hasattr(self, 'episode_initialized'):
            self.episode_initialized = True

            first_sample_idx = data_list[0] if len(data_list) > 0 else 0
            (pos, time, action, pre_pos_count, stay_time) = reward_input_dict[first_sample_idx]
            
            episode_sensitivities = []
            for j in range(len(self.file)):
                with torch.no_grad():
                    reward_j = self.discriminator[j].forward(pos, time, action, pre_pos_count, stay_time)
                    sensitivity_j = self.calculate_sensitivity(reward_j.detach())
                    episode_sensitivities.append(sensitivity_j.item())
    
            total_sensitivity = sum(episode_sensitivities)
            if total_sensitivity > 0:
                weights = [sens / total_sensitivity for sens in episode_sensitivities]
            else:
                weights = [1.0 / len(self.file)] * len(self.file)
            
            self.current_episode_budgets = [episode_budget * weight for weight in weights]
            self.current_episode_sensitivities = episode_sensitivities
            
            # # Debug output (only once per episode)
            # print(f"Episode sensitivities: {episode_sensitivities[:5]}...")
            # print(f"Episode weights: {weights[:5]}...")
            # print(f"Total discriminator budgets: {self.current_episode_budgets[:5]}...")
        
        batch_epsilon_used = 0
        
        for i in data_list:
            (pos, time, action, pre_pos_count, stay_time) = reward_input_dict[i]
            noisy_individual_rewards = []
            sample_epsilon_used = 0
            for j in range(len(self.file)):
                reward_j = self.discriminator[j].forward(pos, time, action, pre_pos_count, stay_time)
                epsilon_j = self.current_episode_budgets[j]
                sensitivity_j = self.current_episode_sensitivities[j]
                if self.noise == 0 or epsilon_j == 0:
                    noisy_reward_j = reward_j
                    actual_epsilon_used = 0
                else:
                    noise_scale_j = sensitivity_j / epsilon_j if epsilon_j > 0 else float('inf')
                    
                    if noise_scale_j != float('inf'):
                        noise_j = torch.tensor(np.random.laplace(0, noise_scale_j, 1), dtype=torch.float32)
                        noisy_reward_j = reward_j.detach() + noise_j
                        actual_epsilon_used = epsilon_j  
                    else:
                        noisy_reward_j = reward_j
                        actual_epsilon_used = 0
                
                sample_epsilon_used += actual_epsilon_used
                noisy_individual_rewards.append(noisy_reward_j)
        
            total_noisy_reward = sum(noisy_individual_rewards)
            avg_noisy_reward = total_noisy_reward / len(self.file)
            batch_epsilon_used += sample_epsilon_used
            reward = avg_noisy_reward
            ...
            reward = torch.abs(reward - self.beta * torch.tensor(value_c, dtype=torch.float32))
            
            log_reward = -torch.log(reward)
            all_result[i] = log_reward.item()
        
        avg_sensitivity = sum(self.current_episode_sensitivities) / len(self.current_episode_sensitivities)
        result_queue.put((all_result, avg_sensitivity, batch_epsilon_used))
    ...
    def get_reward(self, reward_input_dict):
        ...
        episode_epsilon_used = 0
        is_first_process = True
        
        for i in range(result_queue.qsize()):
            results, avg_sens, total_epsilon = result_queue.get()
            reward_dict.update(results)
            sensitivity_values.append(avg_sens)
            
            if is_first_process:
                episode_epsilon_used = total_epsilon
                is_first_process = False
        
        self.total_privacy_budget_used += episode_epsilon_used
            
        if len(sensitivity_values) > 0:
            self.last_avg_sensitivity = sum(sensitivity_values) / len(sensitivity_values)
        else:
            self.last_avg_sensitivity = 0.0
    ...
    def run(self):
        ...
        for i in range(self.episode):
            ...
            while True:
                ...
                value = self.value_net.forward(torch.LongTensor(np.expand_dims(pos, 0)).cuda(), torch.LongTensor(np.expand_dims(time, 0)).cuda(), torch.LongTensor(np.expand_dims(pre_pos_count, 0)).cuda(), torch.LongTensor(np.expand_dims(stay_time, 0)).cuda()).mean().item() #################
                ...
                if done:
                    if buffer_count >= self.train_iter:
                        ...
                        if self.use_adaptive_privacy:
                            avg_sensitivity = np.mean(sensitivity_stats)
                            print(f'Episode {i + 1}, Avg Sensitivity: {self.last_avg_sensitivity:.4f}, Total ε Used: {self.total_privacy_budget_used:.4f}')
                            sensitivity_stats = []  
                        ########## 
                        ...
                        print(f'Episode {i + 1}, Avg Sensitivity: {self.last_avg_sensitivity:.4f}, Privacy Budget Used: {self.total_privacy_budget_used:.4f}')
