...
class Discriminator(nn.Module):
    ...
    #modifiy one
    def forward(self, x_l, x_t, x_a, pre_pos_count, stay_time, return_features=False):
        lemb = self.loc_embedding(x_l)
        temb = self.tim_embedding(x_t)
        aemb = self.act_embedding(x_a)
        semb = self.stay_time_embedding(stay_time)
        ...
        if return_features:
            features = []
            for i, layer in enumerate(self.mlp_layer[:-2]):  # Exclude final sigmoid
                x = layer(x)
                if i % 2 == 1:  # After each ReLU
                    features.append(x)
